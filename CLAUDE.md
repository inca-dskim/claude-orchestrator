...
>
> **v1.4.0 변경사항**:
> - 동적 스킬 생성 정책 추가 (원칙 스킬 vs 구현 스킬)
> - 웹 검색 기반 최신 정보 우선 정책 강화
> - 정적 스킬의 기술별 예시 의존도 감소
>
> **v1.3.0 변경사항**:
> - Phase 5 (프로젝트 코드 생성) 추가
> - 스킬 참조 매핑 추가 (TDD, 클린 아키텍처 등)
> - TDD 적용 워크플로우 및 체크리스트 추가
> - 아키텍처별 디렉토리 구조 명시
>
> **v1.2.0 변경사항**:
...
스킬을 두 가지 유형으로 분리하여 유지보수성과 최신성을 확보합니다:

| 유형 | 내용 | 예시 | 특성 |
|------|------|------|------|
| **원칙 스킬 (정적)** | 변하지 않는 설계 원칙 | TDD 사이클, 클린 아키텍처 원칙, SOLID, DDD | 수년간 유효 |
| **구현 스킬 (동적)** | 기술별 구체적 구현 예시 | Spring Boot 코드, React 패턴, pytest 예시 | 웹 검색으로 생성 |

### 핵심 원칙

1. **원칙 스킬은 정적으로 유지**
   - `tdd-guide`: Red-Green-Refactor 사이클, 테스트 우선 원칙
   - `clean-architecture`: 의존성 역전, 포트와 어댑터 패턴
   - `api-design`: REST 설계 원칙, HTTP 메서드 의미
   - `error-handling`: 예외 계층 설계 원칙

2. **구현 예시는 웹 검색 우선**
   - 정적 스킬의 코드 예시는 **참고용 기본 템플릿**으로만 활용
   - 특정 버전이나 최신 기술은 **반드시 웹 검색**으로 최신 정보 확보
   - 검색 결과 기반으로 **동적으로 코드 생성**

3. **새로운 기술은 전적으로 웹 검색 기반**
   - 스킬에 없는 기술 (Bun, Deno, SvelteKit, SolidJS 등)
   - 공식 문서, 릴리즈 노트 검색 후 즉시 대응

### 동적 스킬 생성 워크플로우

```
1. 사용자: "Spring Boot 4.2 + 클린 아키텍처로 해줘"
                    ↓
2. 원칙 확인: clean-architecture 스킬에서 설계 원칙 참조
                    ↓
3. 웹 검색 트리거:
   - "Spring Boot 4.2 release notes"
   - "Spring Boot 4.2 migration guide"
   - "Spring Boot 4.2 best practices 2026"
                    ↓
4. 동적 구현 생성:
   - 원칙 + 검색 결과 조합
   - 해당 버전의 API/문법 적용
   - 프로젝트 CLAUDE.md에 최신 정보 주입
                    ↓
5. 프로젝트 생성: 항상 최신 베스트 프랙티스 적용
```

### 웹 검색 필수 트리거

다음 상황에서는 **정적 스킬의 예시 코드에 의존하지 않고** 웹 검색을 수행합니다:

- 사용자가 특정 기술/프레임워크 **버전을 명시**한 경우
- 정적 스킬에 해당 기술의 **예시가 없는** 경우
- **최신 베스트 프랙티스**가 필요한 경우
- 지식 컷오프(2025년 5월) **이후 출시** 가능성이 있는 기술

### 기대 효과

- **항상 최신**: 정적 스킬의 레거시화 방지
- **무한 확장**: 스킬에 없는 새로운 기술도 즉시 대응
- **유지보수 감소**: 스킬 파일의 버전별 예시 수동 업데이트 불필요
- **정확성 향상**: 공식 문서 기반 최신 정보 제공
...
사용자가 특정 버전을 명시한 경우, 반드시 다음 프로세스를 따릅니다:

1. **WebSearch로 버전 확인**
   - `[기술명] [버전] release` 검색
   - 공식 릴리즈 여부 확인

2. **버전이 존재하지 않는 경우** (필수 조치)
   - 사용자에게 **즉시 알림**: "요청하신 [기술명] [버전]은 현재 출시되지 않았습니다."
   - 현재 최신 안정 버전 안내
   - **AskUserQuestion으로 확인 요청**:
     > "어떤 버전을 사용하시겠습니까?
     > 1. 최신 안정 버전 [X.Y.Z] 사용 (권장)
     > 2. 베타/RC 버전 [X.Y.Z-beta] 사용
     > 3. 다른 버전 직접 지정"
   - **절대로 임의로 다른 버전으로 대체하지 않음**

3. **버전이 존재하는 경우**
   - 해당 버전의 주요 변경사항/breaking changes 확인
   - 사용자에게 주요 정보 안내 후 진행
...
사용자가 프로젝트 코드 생성을 요청하면, **선택된 옵션에 맞는 스킬을 참조**하여 코드를 생성합니다.

### 5.1 스킬 참조 매핑

사용자가 선택한 옵션에 따라 해당 스킬을 **반드시 참조**합니다:

| 선택 옵션 | 참조할 스킬 | 설명 |
|-----------|-------------|------|
| 클린 아키텍처 (기본) | `clean-architecture` | 헥사고날 아키텍처 패턴, 포트와 어댑터 |
| 레이어드 아키텍처 | `layered-architecture` | Controller-Service-Repository 패턴 |
| TDD (기본) | `tdd-guide` | Red-Green-Refactor 사이클 |
| REST API | `api-design` | URL 설계, HTTP 메서드, 응답 형식 |
| 에러 처리 | `error-handling` | 예외 계층, 글로벌 핸들러, 로깅 |

> ⚠️ **중요: 동적 스킬 생성 정책 적용**
>
> 위 스킬들은 **설계 원칙**을 참조하기 위함입니다.
> 기술별 **구현 예시 코드**는 스킬에 의존하지 않고 **웹 검색**으로 최신 정보를 확보합니다.
>
> - 스킬의 코드 예시: 참고용 기본 템플릿
> - 실제 구현: 웹 검색 결과 기반 동적 생성

### 5.2 TDD 적용 워크플로우 (필수)

TDD가 선택된 경우 (기본값), 다음 순서를 **반드시** 따릅니다:

```
1. RED: 실패하는 테스트 먼저 작성
   └── 예: UserTest.java 작성 → 테스트 실행 → 실패 확인

2. GREEN: 테스트를 통과하는 최소 코드 작성
   └── 예: User.java 구현 → 테스트 실행 → 통과 확인
...
**TDD 적용 체크리스트:**
- [ ] 기능 구현 전 테스트 코드 먼저 작성했는가?
- [ ] 테스트가 실패하는 것을 확인했는가? (RED)
- [ ] 테스트를 통과하는 최소 코드를 작성했는가? (GREEN)
...
1. **도메인/엔티티** → 테스트 먼저, 구현 후
2. **서비스/유스케이스** → 테스트 먼저, 구현 후
3. **컨트롤러/어댑터** → 테스트 먼저, 구현 후
4. **설정 파일** (application.yml, build.gradle 등)

### 5.5 생성 완료 확인

프로젝트 코드 생성 완료 시 다음을 확인합니다:

- [ ] 선택한 아키텍처에 맞는 디렉토리 구조인가?
- [ ] TDD 순서대로 테스트가 먼저 작성되었는가?
- [ ] 모든 테스트가 통과하는가?
- [ ] api-design, error-handling 패턴이 적용되었는가?
...
**프로젝트 코드 생성용 스킬 (Phase 5에서 참조):**
- `tdd-guide`: TDD 적용 시 참조 (기본값)
- `clean-architecture`: 클린 아키텍처 선택 시 참조 (기본값)
- `layered-architecture`: 레이어드 아키텍처 선택 시 참조
- `api-design`: REST API 설계 시 참조
- `error-handling`: 에러 처리 설계 시 참조
